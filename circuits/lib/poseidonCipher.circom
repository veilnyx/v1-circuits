...
include "../../node_modules/@zk-kit/poseidon-cipher.circom/src/poseidon-constants-old.circom";

// we copy functions like Ark, Mix, Sigma from "poseidon_old.circom"
template _Sigma() {
    signal input in;
    signal output out;
...
template PoseidonPerm(nInputs) {
    ...
    for (var i=0; i<nRoundsF + nRoundsP; i++) {
        ark[i] = _Ark(nInputs, C, nInputs*i);
